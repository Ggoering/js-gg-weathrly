...
this.state = {
  ...
  componentDidMount() {
    for (let i = 0; i < 7; i++) {
    this.state.sevenHour.push({hour: dummyData.hourly_forecast[i].FCTTIME.hour,
    temp: dummyData.hourly_forecast[i].temp.english})
    this.setState(this.state.sevenHour)
    }
    for (let i = 0; i < 10; i++) {
    this.state.tenDay.push({dayName: dummyData.forecast.simpleforecast.forecastday[i].date.weekday, high: dummyData.forecast.simpleforecast.forecastday[i].high.fahrenheit , low: dummyData.forecast.simpleforecast.forecastday[i].low.fahrenheit})
  ...
  console.log(this.state.tenDay);
    return (
      <main style={appStyle}>
        ...
        <HourlyForecast hourData={this.state.sevenHour}/>
        <TenDayForecast weeklyData={this.state.tenDay}/>
      </main>
    )
  }
}
